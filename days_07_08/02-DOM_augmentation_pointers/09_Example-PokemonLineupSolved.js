  ...
  allTiles[i].onclick = function(event) {
    ...
    if (event.currentTarget.parentElement === available) {
      ...
      if (slot1.children.length === 0) {
        event.currentTarget.remove(); // don't technically need, but good practice to include
        // append child  automatically removes from its current parent and a  dds to the new one
